...
public struct TLPHAsset {
    ...
    var limitedPermission: Bool = false
...
public struct TLAssetsCollection {
    ...
    var idAsset: [String: TLPHAsset] = [String: TLPHAsset]()
    ...
    func getTLAsset(at indexPath: IndexPath) -> TLPHAsset? {
        ...
        if let sections = self.sections {
            ...
            guard let result = self.fetchResult, index < result.count else { return nil }
            let asset = result.object(at: max(index,0))
            if let tlpHAsset = idAsset[asset.localIdentifier] {
                return tlpHAsset
            }
    ...
    mutating func cachedTLPHAsset(localIdentifier: String, for tlpHAsset: TLPHAsset?) {
        self.idAsset[localIdentifier] = tlpHAsset
    }
    
    mutating func clearTLPHAssetCached() {
        self.idAsset.removeAll()
    }

...
public struct TLPhotosPickerConfigure {
    ...
    public var selectMessage = "Select".localizedString()
    public var deselectMessage = "Deselect".localizedString()
...
open class TLPhotosPickerViewController: UIViewController {
    ...
    private(set) var photoLibrary = TLPhotoLibrary()
    ...
    public func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        DispatchQueue.main.async {
            ...
            if changes.hasIncrementalChanges, self.configure.groupByFetch == nil {
                ...
                self.focusedCollection?.clearTLPHAssetCached()
                self.focusedCollection?.fetchResult = changes.fetchResultAfterChanges
                self.reloadCollectionView()
            }
...
extension TLPhotosPickerViewController: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDataSourcePrefetching {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard var asset = collection.getTLAsset(at: indexPath) else { return cell }
        self.focusedCollection?.cachedTLPHAsset(localIdentifier: asset.localIdentifier, for: asset)
        ...
        if let phAsset = asset.phAsset {
            if(asset.completed) {
                cell.imageView?.image = asset.imageCached
                ...
                cell.limitedPermission = asset.limitedPermission
            } else {
                    ...
                    let requestID = self.photoLibrary.imageAsset(asset: phAsset, size: self.thumbnailSize, options: options) { [weak self, weak cell] (image,complete) in
                        ...
                        DispatchQueue.main.async {
                            if self.requestIDs[indexPath] != nil {
                                ...
                                if complete {
                                    self.requestIDs.removeValue(forKey: indexPath)
                                    asset.limitedPermission = (image == nil)
                                    asset.completed = true
                                    asset.imageCached = image
                                    self.focusedCollection?.cachedTLPHAsset(localIdentifier: asset.localIdentifier, for: asset)
                                    cell?.limitedPermission = asset.limitedPermission
                                }
                            }
...
extension TLPhotosPickerViewController {
    ...
    func toggleSelection(for cell: TLPhotoCollectionViewCell, at indexPath: IndexPath) {
        ...
        if(asset.limitedPermission == true) {
            let alertController = UIAlertController(title: "Photo is limited access".localizedString(), message: "You want to reselection ?".localizedString(), preferredStyle: UIAlertController.Style.alert)
            let a1 = UIAlertAction(title: "Reselect".localizedString(), style: UIAlertAction.Style.default) {[weak self] (alertAction) in
                self?.limitButtonTap()
            }
            alertController.addAction(a1)
            alertController.addAction(UIAlertAction(title: "Cancel".localizedString(), style: UIAlertAction.Style.cancel, handler: nil))
